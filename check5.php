...
if(isset($_POST['check']))
{
	 ...
	 //Since,e2 picks only 1 column,so naturally res which fetches e2 will have to be res[0]
...
$q8="select `type` from `products` where name='$b'";
$e8=mysqli_query($x,$q8);
$res6=mysqli_fetch_row($e8);
$q9="select `name` from `transactions1` where trans_no='$res[0]'";
$e9=mysqli_query($x,$q9);

if(mysqli_num_rows($e3)>0)
{	
while($res1=mysqli_fetch_row($e3))
{
$q11="select `type` from `products` where name='$res1[3]'";
$e11=mysqli_query($x,$q11);
$res5=mysqli_fetch_row($e11);
$z=$res1[5]/$res1[4];
if($res1[3]!=$b)
{
if($res6[0]==$res5[0])
